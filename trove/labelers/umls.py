...
UMLS_VERS_MD5_CHECKSUMS = {
    "cf0699e5ca95d9eabc1a0b3a7f1bfda7": {
        "release": "full",
        "year": 2020,
        "version": "AB",
    },
    "9bca7282fa4ca6d0544e87628da67ddf": {
        "release": "full",
        "year": 2018,
        "version": "AA",
    },
...
class UMLS:
    ...
    def __init__(self, **kwargs):
        ...
        self.backend = kwargs["backend"] if "backend" in kwargs else UMLS.backend
        self.cache_path = UMLS.get_full_cache_path(
            kwargs["cache_path"] if "cache_path" in kwargs else UMLS.cache_root
        )
    ...
    def _load_indices(self):
        ...
        self.langs = {
            sab: lang
            for sab, lang, ssn in msgpack.load(
                open(f"{self.cache_path}/sabs.bin", "rb")
            ).values()
        }
        self.tui_to_sty = msgpack.load(open(f"{self.cache_path}/tui_to_sty.bin", "rb"))
    ...
    def _load_terminologies(self, filter_sabs, type_mapping="TUI"):
        ...
        if self.backend == "pandas":
            df = pd.read_parquet(
                f"{self.cache_path}/concepts/", engine="pyarrow"
            ).groupby(["SAB"])
    ...
    def _apply_filters(
        ...
        def include(t):
            return (
                t
                and len(t) >= min_char_len
                and t.count(" ") <= max_tok_len - 1
                and t not in stopwords
                and (filter_rgx and not filter_rgx.search(t))
            )
    ...
    def init_sqlite_tables(fpath, dataframe):
        ...
        conn.executemany(
            "INSERT into terminology(sab, tui, cui, term) values (?,?,?,?)", rows
        )
        conn.commit()
        conn.close()
    ...
    def is_initalized(cache_root=None, backend=None):
        ...
        flags = [os.path.exists(f"{cache_path}/{fname}") for fname in filelist]
    ...
    @staticmethod
    def init_from_nlm_zip(
        fpath, outdir=None, backend=None, use_checksum=False, keep_original_rrfs=False
    ):
        """
        ...
        if use_checksum:
            ...
            if checksum not in UMLS_VERS_MD5_CHECKSUMS:
                print("WARNING - UMLS checksum not found")
                use_checksum = False
            else:
                print(f"Detected UMLS version {UMLS_VERS_MD5_CHECKSUMS[checksum]}")
        ...
        if not keep_original_rrfs:
            shutil.rmtree(tmp)
            logger.info("Deleted temporary UMLS files at %s", tmp)
    ...
    def init_from_rrfs(indir, outdir=None, backend=None):
        ...
        df = pd.read_csv(
            ...
            na_filter=False,
            dtype={"SAB": "object", "TUI": "object", "CUI": "object", "TERM": "object"},
        )
