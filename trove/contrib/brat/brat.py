...
class BratBase(object):
    ...
    def __init__(self, id_, type_, doc_name):
        self.id_ = id_
        self.type_ = type_
...
class StandoffParser(object):
    ...
    def _parse_annotations(self, txt_filename, ann_filename):
        ...
        with codecs.open(ann_filename, "rU", encoding=self.encoding) as fp:
            for line in fp:
                ...
                if anno_id_prefix == StandoffParser.TEXT_BOUND_ID:
                    ...
                    annotations[anno_id] = Entity(anno_id, doc_name, entity_type, ((i, j),), mention)
        ...
        return list(set(annotations.values()))
...
class BratAnnotations(object):
    ...
    def annotator_agreement(self, class_map, ignore_types=[], relations_only=False, method="randolph"):
        """
        ...
        annos = {c:votes for c,votes in annos.items() if c.type == 'Complication'}# and class_map(c) == 1}
     
        # overall score
        V = np.vstack(annos.values())
      
        X = np.vstack([annos[m] for m in annos])
        
        x = X[...,0]
        print('Positive 3', len(x[x == 3]))
        print('Positive 2', len(x[x == 2]))
        print('Positive 1', len(x[x == 1]))
        print( len(x) )
    ...
    def get_labels(self, candidates, ignore_attributes=True, neg_label=0):
        ...
        if ignore_attributes:
            ...
            gold = self.labels
        return np.array([1 if c in gold and gold[c] == 1 else neg_label for c in brat_cands])

    def score(self, candidates, pred_labels, ignore_attributes=True, neg_label=0):
        ...
        brat_cands = snorkel_to_brat(candidates, ignore_attributes)
        brat2snorkel = dict(zip(snorkel_to_brat(candidates, ignore_attributes), candidates))
        ...
        for i, c in enumerate(brat_cands):
        ...
        ooc = 0
        for c in gold:
            if c not in brat_cands and gold[c] == 1:
                ooc += 1
